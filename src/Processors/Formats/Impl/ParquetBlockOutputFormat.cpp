...
void ParquetBlockOutputFormat::consume(Chunk chunk)
{
    ...
    /// accumulated chunk into multiple row groups.
    if (accumulated_rows >= target_rows * 2)
    {
        /// Increase row group size slightly (by < 2x) to avoid a small row group at the end.
        size_t num_row_groups = std::max(static_cast<size_t>(1), accumulated_rows / target_rows);
        size_t row_group_size = (accumulated_rows - 1) / num_row_groups + 1; // round up

        Chunk concatenated = std::move(accumulated_chunks[0]);
        for (size_t i = 1; i < accumulated_chunks.size(); ++i)
            concatenated.append(accumulated_chunks[i]);
        accumulated_chunks.clear();

        for (size_t offset = 0; offset < accumulated_rows; offset += row_group_size)
        {
            size_t count = std::min(row_group_size, accumulated_rows - offset);
    ...
    {
        writeRowGroup(std::move(accumulated_chunks));
    }
...
void ParquetBlockOutputFormat::writeRowGroup(std::vector<Chunk> chunks)
{
    ...
    if (format_settings.parquet.use_custom_encoder)
        ...
        writeUsingArrow(std::move(concatenated));
}

void ParquetBlockOutputFormat::writeUsingArrow(Chunk chunk)
{
    ...
    auto status = file_writer->WriteTable(*arrow_table, chunk.getNumRows());
